---
title: "EKKOLearnAI/hermes-studio: Web dashboard for Hermes Agent — multi-platform AI chat, session management, scheduled jobs, usage analytics"
source: "https://github.com/EKKOLearnAI/hermes-studio/blob/main/README_zh.md"
author:
published:
created: 2026-07-25
description: "Web dashboard for Hermes Agent — multi-platform AI chat, session management, scheduled jobs, usage analytics  - EKKOLearnAI/hermes-studio: Web dashboard for Hermes Agent — multi-platform AI chat, session management, scheduled jobs, usage analytics"
tags:
  - "clippings"
taxonomy: { doc_category: [hermes] }
---
**Hermes Studio** [English](https://github.com/EKKOLearnAI/hermes-studio/blob/main/README.md)

面向 [Hermes Agent](https://github.com/NousResearch/hermes-agent) 的桌面应用、本地运行时和 Web 控制台。  
聊天、模型与 Profile 管理、平台渠道接入、任务自动化、  
文件查看、Coding Agent 和本地运行环境都在一个界面中完成。

[下载 Hermes Studio 桌面版](https://github.com/EKKOLearnAI/hermes-studio/releases/latest) · `npm install -g hermes-web-ui && hermes-web-ui start`

[![Hermes Web UI 演示](https://github.com/EKKOLearnAI/hermes-studio/raw/main/packages/client/src/assets/image.gif)](https://github.com/EKKOLearnAI/hermes-studio/blob/main/packages/client/src/assets/image.gif)
...
- 支持的设置项、安全边界和当前非目标范围见 [`docs/voice-dialogue.md`](https://github.com/EKKOLearnAI/hermes-studio/blob/main/docs/voice-dialogue.md) 。
...
从 [GitHub Releases](https://github.com/EKKOLearnAI/hermes-studio/releases/latest) 下载最新的 **Hermes Studio** 桌面安装包。
...
桌面壳自身的 Web UI 状态会单独保存到 `~/.hermes-web-ui` ，除非设置了 `HERMES_WEB_UI_HOME` 。
...
使用 `hermes-studio cli -h` 查看 Hermes Agent CLI 帮助，使用 `hermes-studio web -h` 查看 Web UI CLI 帮助。

桌面自动更新会优先读取 `https://download.ekkolearnai.com/latest` 。 如果该端点不可用，更新器会回退到 `https://github.com/EKKOLearnAI/hermes-studio/releases/latest/download` 。
...
更详细的说明与排错见： [`docs/docker.md`](https://github.com/EKKOLearnAI/hermes-studio/blob/main/docs/docker.md)

### Hermes Agent 运行时发现

Web UI 启动后端聊天能力时，会优先使用包含 `run_agent.py` 的源码目录，例如 `~/.hermes/hermes-agent` 。如果找不到源码目录，会退回到已安装 `hermes` 命令所使用 的 Python 环境，再退到系统 Python。因此源码安装和 `pip install hermes-agent` 这类 包安装方式都可以兼容。
...
npm run dev
```
- 前端： [http://localhost:8649](http://localhost:8649/)
- BFF 服务器： [http://localhost:8647](http://localhost:8647/)
```applescript
npm run build   # 构建输出到 dist/
```

项目开发规范见： [DEVELOPMENT.md](https://github.com/EKKOLearnAI/hermes-studio/blob/main/DEVELOPMENT.md) 。
...
[BSL-1.1](https://github.com/EKKOLearnAI/hermes-studio/blob/main/LICENSE)

该许可证覆盖 Hermes Studio、原 Hermes Web UI 名称、 `hermes-web-ui` npm 包和 CLI、桌面应用、固件、发布产物、文档以及本仓库内的关联文件。