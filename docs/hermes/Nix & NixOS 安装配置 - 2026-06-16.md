...
执行 `nix profile install` 后， `hermes` 、 `hermes-agent` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与 [标准安装](https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/installation) 完全相同—— `hermes setup` 引导你完成提供商选择， `hermes gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.hermes/` 。
...
> [!-danger] -danger
> 切勿将 API 密钥放入 `settings` 或 `environment`
> 
