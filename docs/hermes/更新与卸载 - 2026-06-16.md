...
1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.hermes/pairing/` 、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/checkpoints-and-rollback) 中描述的快照恢复流程进行恢复，或从 Hermes 写入 `~/.hermes/` 目录旁的最新快速快照 zip 文件中提取。
...
详情参见 [Nix 安装](https://hermes-agent.nousresearch.com/docs/zh-Hans/getting-started/nix-setup) 。
