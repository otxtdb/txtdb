...
注意：有可能出现接口是对的，可是网站主机的PHP环境的某些模块没有开启也会导致无法显示二维码，目前为止[模板兔](https://www.mobantu.com/ "模板兔")帮用户调试了部分主机，发现有些主机（cPanel）PHP环境没有开启http模块，经开启后解决了问题，部分windows主机环境不兼容支付宝当面付接口。